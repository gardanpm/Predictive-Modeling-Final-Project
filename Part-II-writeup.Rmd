...
```{r boosting}
model_boosting = gbm(logprice ~  - Surface + log(Surface + 1) + year + diff_origin + Interm + engraved + prevcoll + finished + lrgfont + dealer + lands_sc + peasant + portrait + still_life + discauth + materialCat + endbuyer + origin_author +Shape + artistliving,
         data = paintings_train_subset,
                     distribution = "gaussian",
                     n.trees = 1000)
summary(model_boosting, plotit = F)

predict_boosting_train = predict(model_boosting, newdata = paintings_train_subset, n.trees = 1000,type = "response")
predict_boosting_train = exp(predict_boosting_train)
rmse_boosting_train = rmse(predict_boosting_train, exp(paintings_train_subset$logprice))
rmse_boosting_train 
plot(exp(paintings_train_subset$logprice), predict_boosting_train)
```
...
predictions = as.data.frame(
  exp(predict(model_boosting, newdata = paintings_train_subset, n.trees = 1000,
              interval = "pred")))
