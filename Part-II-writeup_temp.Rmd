...
full_train <- paintings_train %>%
  ...
  select(logprice, Surface, year, nfigures, relig,
         ...
         pastorale, Interm, engraved, prevcoll, finished,
         lrgfont, lands_sc, portrait, still_life,
         discauth, artistliving, origin_cat,
         origin_author, materialCat, school_pntg,
         diff_origin, endbuyer,engraved, nfigures,
         #additional
...
```{r aic selection}
#full_lm <- lm(logprice ~ (. - Surface + log(Surface) -year - lowest_winningBidder - lowest_authors
 #                         - highest_winningBidder - highest_authors)^2 + year + lowest_winningBidder + lowest_authors
  #                        + highest_winningBidder + highest_authors, data = train)
