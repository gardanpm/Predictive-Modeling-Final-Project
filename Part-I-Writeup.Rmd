...
paintings_train_subset <- paintings_train %>% 
  mutate(lSurface = log(Surface + 1)) %>% 
  select(logprice, year, lSurface, diff_origin, Interm, engraved, prevcoll,
         paired, figures,finished, lrgfont, lands_s, peasant, portrait, still_life,
         discauth, dealer, materialCat, endbuyer, Shape, school_pntg, 
         lSurface:materialCat, school_pntg:diff_origin,
         dealer:school_pntg, dealer:Interm, dealer:endbuyer, endbuyer:Interm,
         finished:prevcoll)


model.init <- lm(logprice ~ . ,data = impute_NA(paintings_train_subset))

texreg(model.init)
```

```{r model selection, results = "asis"}
model.all <- lm(logprice ~ . ^ 2,data = impute_NA(paintings_train_subset))
model.sel <- step(model.init, k=2)
texreg(model.init)
```
