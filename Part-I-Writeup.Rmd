...
summary(lm(logprice~endbuyer, data=paintings_train))
...
```{r model selection}
# call impute functon here
# Initial full model
# Stepwise selection (AIC/BIC/backwards/both/etc.)
# Anova
#lm(logprice ~ year + log(Surface+1) + diff_origin + Interm +
  #                     engraved + prevcoll + paired + figures +finished +
   #                    lrgfont + lands_sc + peasant + portrait + still_life +
    #                   discauth + dealer + materialCat + endbuyer + Shape +
                       school_pntg +
                       log(Surface+1):materialCat + school_pntg:diff_origin +
                       dealer:school_pntg + dealer:Interm + dealer:endbuyer +
                       endbuyer:Interm + finished:prevcoll 

paintings_train_subset <- paintings_train %>% select(logprice, year, Surface,  diff_origin, Interm ,  engraved , prevcoll , paired , figures ,finished ,
                      lrgfont , lands_sc , peasant , portrait , still_life ,
                      discauth , dealer , materialCat , endbuyer , Shape ,
                       school_pntg)


model.init <- lm(logprice ~ (. - Surface + log(Surface+1))^2 ,data = impute_NA(paintings_train_subset))
